pipeline {
	...
	environment {
		DOCKER_IMAGE = "nastss6tx5/asset-spy-auth-service"
		DOCKER_TAG = "${env.BUILD_NUMBER}"
		GITHUB_REPO = "Nastss6tx5/asset-spy-auth-service"
				...
				stage('Deploy to Kubernetes') {
					steps {
						...
						withCredentials([file(credentialsId: KUBECONFIG_CREDENTIALS_ID, variable: 'KUBECONFIG')]) {
							sh '''
							kubectl -n asset-spy set image deploy/auth-service auth-service=${DOCKER_IMAGE}:${DOCKER_TAG}
							kubectl -n asset-spy rollout status deploy/auth-service --timeout=300s
							'''
						}
					}
				}
